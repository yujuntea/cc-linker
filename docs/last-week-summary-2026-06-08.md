...
- **三级内容降级**（`resolvePeekContent`）：
  1. **Own JSONL** → 直接读目标 session 的 JSONL 文件最后一条 assistant 文本
  2. **Parent JSONL** → roster resume-from 场景（session 是从父 session 派生的），读父 session JSONL 中对应片段
  3. **`claude logs` fallback** → 最后兜底，此时用 terminal code-block 渲染（仅此路径有 box-drawing 字符问题）
