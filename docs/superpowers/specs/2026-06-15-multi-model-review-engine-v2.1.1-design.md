...
| 能力 | 复用什么 | v2.1 验证状态 |
|------|---------|--------------|
| Spawn bg session（work / 单一 review） | `Bun.spawn(['claude', '--bg', prompt, '--settings', settingsPath])` → 返回 `shortId` 落到 `~/.claude/jobs/<short>/state.json` | ✅ 实测 OK（见附录 A） |
...
├── adapter.ts           # v2.1 重写：ClaudeBGAdapter 暴露 5 个 API：
...
stateDiagram-v2
    ...
    note right of PANE_LOST
      ...
      - lostPanes: array of { role, shortId } (supports multiple panes lost simultaneously)
      - detectedAt: ISO timestamp
      - retryTarget: remembered state to restore to

      Three user choices via review status command:
      - retry: restore to retryTarget state, respawn all lost panes
...
| **任意状态 X** | bg session 消失 (Reconciler) | PANE_LOST | 记录 `lostPanes` + `detectedAt`（支持多 pane 同时丢失） | lost pane(s) | 不变 |
...
export async function reconcile(): Promise<void> {
  ...
  for (const record of await store.listRunning()) {
    ...
    // - role: 'work' | 'review-A' | 'review-B' | ...（从 record.panes 拿）
...
async function retryPANE_LOST(pipelineId: string): Promise<void> {
  ...
  const { lostPanes, retryTarget } = record.state;

  // 1. 清理所有丢失 pane 的 state（如果还在）
  for (const lp of lostPanes) {
    try { await adapter.stop(lp.shortId); } catch { /* 已被 daemon reap */ }
  }

  // 2. 重新 spawn 所有 lost pane（用 retryTarget 决定启动方式）
  if (retryTarget === 'EXTERNAL_REVIEW') {
    // review pane：找到原 provider + prompt template，重新 `--bg` spawn
    for (const lp of lostPanes) {
      const originalPane = record.panes.reviews.find(r => r.role === lp.role);
      ...
      record.panes.reviews = record.panes.reviews.map(r =>
        r.role === lp.role ? { ...r, shortId: newShortId } : r
      );
    }
  } else if (retryTarget === 'FIXING' && lostPanes.some(lp => lp.role === 'work')) {
    ...
    throw new Error(`retry not supported for retryTarget=${retryTarget} lostPanes=${lostPanes.map(p => p.role).join(',')}`);
  }
...
```typescript
function parseBgOutput<T>(output: string, schema: z.ZodSchema<T>): { ok: true; data: T } | { ok: false; raw: string } {
  try {
    const json = extractJsonBlock(output);
    const data = schema.parse(json);  // Zod validate
    return { ok: true, data };
  } catch (err) {
    logger.warn(`[engine] output parse failed: ${err.message}`);
...
| 场景 | 处理 |
|------|------|
| JSON 提取失败（无 fence、无 {} 块） | raw response 存为 history event 的 `outputDigest`；该轮视为 **0 issues** |
| JSON.parse 成功但 Zod validate 失败 | 同上 + warn 日志含 schema mismatch 详情 |
| Review pane JSON parse 失败 | 该 review 视为 0 issues，其他 review 正常处理 |
| Work session R1/R2 parse 失败 | 视为 0 issues → 如果其他 review 也没有 issues → DONE |
| Work session JUDGE parse 失败 | 视为全部 accept → 进入 FIXING（保守：让外部 review 通过） |
| Work session FIXING parse 失败 | 视为 all_real_fixed=true（保守：认为修复已完成）→ 推进到下一状态 |
...
#### 7.5.7 Context Window 膨胀风险

Work session 跨多个 round 共享同一个 sessionId，context 不断膨胀：

| Round | 累计注入次数 | 预估 context tokens |
|-------|------------|-------------------|
| 1 | 6-8 次 | ~30-50k |
| 3 | 18-24 次 | ~90-150k |
| 8 (Code max) | 48-64 次 | ~200-400k |

Claude context window 是 200k tokens。到 round 5+ 时，早期内容可能被自动压缩或截断。

**缓解策略**：
1. **Prompt 自包含**：每轮 R1/R2 prompt 包含当前 artifact 的最新版本（通过 `resolvePeekContent` 获取），不依赖 session context 中的旧版本
2. **Issues 摘要传递**：FIXING→R2 转换时，engine 在 prompt 中显式列出 "本轮需要验证的 issues"，不让 work session 自己从 context 中回忆
3. **监控 + 降级**：如果 `state.json` 中有 token usage 信息（`usage` 字段），engine 在接近 200k 时 log warn；超过时不强制中断，但 R1 可能不再发现新 issues（自然收敛）
4. **max_rounds 是天然上限**：Code=8 轮时 context 可能溢出，但 R1 不再发现 issues → 自然 DONE
...
| **Context window 膨胀** | work session 跨 8 轮 ~48 次注入，context 可能超过 200k tokens，导致早期内容被压缩/丢失，R1/R2 质量下降 | Prompt 自包含（每轮注入最新 artifact 摘要）+ Issues 显式传递 + max_rounds 是天然上限（R1 不再发现 issues → 自然 DONE）。详见 §7.5.7 |
