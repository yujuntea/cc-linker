...
cc-linker review run <task>     [--phase spec|plan|code] [--profile default] [--max-rounds N] [--watch] [--cwd <path>] [--inject-text "补充 prompt"] [--max-cost 5.00]
...
- **v2.1.1 P0-C1**：新增 `review skip`（用户手审完后温和收尾，标 DONE 但保留 history）
- **v2.1.1 P0-C3**：`run` 加 `--inject-text`（补充 prompt，不修改 task 本身）
- **v2.1.1 P0-C4**：`run` 加 `--max-cost`（cost 硬上限）
- **v2.1.1 P0-C8**：`report` 加 `--partial`（中途看 Markdown 报告）
...
**v2.1.1 P0-C6 修复：HUMAN_DECIDE issue 列表排版模板**

```bash
# 1. pipeline 进入 HUMAN_DECIDE 后，CLI 输出（v2.1.1 P0-C6 改进模板）：
#   ⏸️ Pipeline 01HXYZK9... 等待人工决策（默认 4h 超时，来自 profile.guards.human_decide_timeout_ms）
#
#   === Issues by Severity (default sort) ===
#   [P0] 3 issues
#     1. auth.ts:42 - NPE in validateUser() - missing null check (review-1)
#     2. auth.ts:78 - Race condition in login flow (review-2)
#     3. db.ts:120 - SQL injection in query builder (review-3)
#   [P1] 2 issues
#     4. utils.ts:45 - Deprecated function usage (review-4)
#     5. api.ts:200 - Missing error handling in fetchUser (review-5)
#   [P2] 1 issue
#     6. config.ts:15 - Hardcoded timeout value (review-6)
#
#   Work verdict: reject (P0/P1 rejection ratio ≥ 30%)
#
#   === Actions ===
#     a) 接受所有 issue → 修复: cc-linker review decide 01HXYZK9... --accept-all
#     b) 接受子集       → 修复: cc-linker review decide 01HXYZK9... --accept "1,3,5"
#     c) 拒绝所有      → 中止: cc-linker review decide 01HXYZK9... --reject-all
#     d) 推迟          → 等你: 什么都不做，4h 后自动 ABORTED
#     e) 按文件分组查看 → 输出 issue list grouped by file
#     f) 查看完整 Markdown 报告 → cc-linker review report 01HXYZK9... --partial
#
#   提示: 输入 `cc-linker review status 01HXYZK9... --interact` 进入交互模式

# 2. 用户决策后，pipeline 自动继续
```

**Issue 列表排序规则**（v2.1.1 P0-C6）：
- 默认按 severity desc → location asc → issue id asc
- 超过 20 issues 时按 file group 折叠（`--expand file X` 展开某个 file）
- 每条 issue 显示：`[severity] file:line - description (issue_id)`

**v2.1.1 P0-C7 修复：错误信息模板加"重试 / 找帮助"链接**

```bash
# 错误信息模板（v2.1.1 P0-C7 改进）：
#   ❌ <错误类型>
#   <具体错误描述>
#
#   💡 下一步:
#     - 重试: cc-linker review run <task> --profile default
#     - 查看诊断: cc-linker review doctor
#     - 查看日志: tail -f ~/.cc-linker/review-engine.log
#     - 寻求帮助: https://github.com/yujuntea/cc-linker/issues
#
#   错误详情: <stack trace 或 error code>
```
