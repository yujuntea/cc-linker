...
**日期：** 2026-06-19
**版本：** v1.1（基于当前 master `2f6c6b3`）
**状态：** 待评审
**作者：** Claude Code（brainstorming + 用户拍板）
**范围：** 新增企业微信智能机器人通道，作为飞书的并行平台

### v1.1 修订（2026-06-19 eng review 后）

- **§3.2 设计原则**：澄清 `StreamUpdater` 接口只覆盖普通聊天流式 5 个核心方法；飞书 v2.4.x Agent View / rendezvous 专属方法不在接口内，飞书侧继续直接用 `CardUpdater`
- **§3.2 第 1 条**：原"核心不动只换消息类型"改为"语义不动、字段增量化"——`SpoolMessage` 保留 `openId/text`（46 个调用方无感），新增 `userId/platform` 字段
- **§3.2 第 3 条**：明确 PR 1 末尾必须完成飞书侧切换到 `FeishuStreamUpdater`（否则 PR 1 之后 `FeishuStreamUpdater` 类是死代码）
- **§3.2 第 4 条（新增）**：UserManager 抽到 `platform/user-state.ts` 但保留 `feishu/mapping.ts` 兼容层（re-export），不再重写 UserManager
- **§3.3（新增）**：SpoolMessage 跨平台兼容策略——新字段 + alias + 渐进迁移
- **§3.4（新增）**：StateCoordinator 多 daemon 并存语义——单平台锁 + 独立 lock 文件
- **§4.1 PlatformMessage**：`chatId` / `text` 字段加注释说明（feishu p2p 用 open_id 兜底；text 在 feishu 路径是原始 JSON string，下游 bot.ts 才 parse）
- **§4.1 StreamUpdater 接口**：从 spec 自创的 `start/update/finish/fail` 改为对齐 CardUpdater 真实形状的 5 方法（`startProcessing/updateStream/complete/error/cancel`）
- **§4.2 aibot-client**：SDK error 处理从 listener 内 `throw` 改为 `emit('fatal', CCLinkerError)`，避免 uncaught exception 绕过 handleError
- **§4.2 stream-updater**：`complete/error/cancel` 三个终态方法前置 `prepareTerminal` 处理（guard + clearTimeout），避免 timer fire 覆盖终态消息；flushBuffer 只吞限频错误，其他 rethrow
- **§6.2 A0**：错误处理描述更新，明确 `E_CONFIG_WECOM_AUTH` / `E_CONFIG_WECOM_NETWORK` 两条错误码
- **§9.1 风险表**：WSAuthFailureError 误显示风险标 ✅ 已修复；新增 `UserManager 重复实现` 风险（✅ 已修复）；新增 `SpoolMessage 改名破坏 46 个调用方` 风险（✅ 已修复 v1.1 §3.3）；新增 `StateCoordinator 多 daemon 语义未明` 风险（✅ 已修复 v1.1 §3.4）
...
1. **核心"语义不动、字段增量化"**：SpoolQueue / SessionManager / Registry / StateCoordinator **核心算法不动**；`SpoolMessage` 在飞书路径下保留 `openId` / `text` 字段（46 个调用方无感），**新增** `userId` + `platform` 字段作为平台无关入口。企微路径写入时 `userId = externalUserid`，`platform = 'wecom'`，`openId` 留空或写 alias（飞书兼容）。
2. **抽两个接口，不抽全部**：只抽"消息"和"流式更新"，其他（队列 worker 池、CAS、权限处理）保留飞书路径。`StreamUpdater` 接口只覆盖"普通聊天流式"5 个核心方法（`startProcessing / updateStream / complete / error / cancel`）；飞书 v2.4.x Agent View / rendezvous 专属方法（`setRendezvousShortId / patchWaitingCard / cancelPending / patchAbortedTracking / adoptExistingCard`）不在接口内，飞书侧这些代码继续直接用 `CardUpdater` 类型
3. **飞书路径零回归（PR 1 强制）**：飞书的 `bot.ts` 普通聊天路径走 `FeishuStreamUpdater` 适配新接口；Agent View / rendezvous 路径仍直接用 `CardUpdater` 完整能力。两条路径在 bot.ts 内并存。**PR 1 末尾必须完成飞书侧切换**（否则 `FeishuStreamUpdater` 类是死代码，到 PR 2 第一次用时才发现类型不匹配）
4. **UserManager 抽到 platform/user-state.ts 但保留 feishu/mapping.ts 兼容层**：`feishu/mapping.ts` 已有 `UserManager` 实现（含 `compareAndSwap` / `rollbackTimedOutClaims` / `validateOwner` / `allEntries` 等 8 个方法，~30 个 import 方），**不重写**。在 `platform/user-state.ts` 放接口契约 + 跨平台抽象；`feishu/mapping.ts` 改为 `re-export { UserManager } from '../platform/user-state'` + `PlatformUserManager` 扩展子类（添加 `validateOwnerByConfigKey('feishu_bot.owner_open_id')` 等钩子）；`wecom/mapping.ts` 用 `PlatformUserManager` + 独立 storage path + override `validateOwner`
5. **企微路径最简**：直接复用 OpenClaw 官方 SDK `@wecom/aibot-node-sdk`（MIT 协议）+ 薄包装

### 3.3 SpoolMessage 跨平台兼容策略（v1.1 新增）

**事实**：`SpoolMessage.openId` / `text` 在 46 个文件中被引用（`spool.ts:30-48`，`feishu/bot.ts` 全文），完全重命名为 `userId` / `content` 会触发 46 个文件级联改 + 大量单测失败。

**策略**：
- `SpoolMessage` **新增** 2 个字段：
  - `userId: string` —— 平台无关用户 ID（飞书=open_id；企微=external_userid）
  - `platform: 'feishu' | 'wecom'` —— 标记来源
- `openId` / `text` **保留** 为飞书侧 alias 字段：
  - 飞书路径 `feishu/bot.ts:onMessage` 入队时同时写 `userId = open_id, platform = 'feishu'` 和 `openId = open_id, text = event.content`（两份等价数据，渐进迁移）
  - 企微路径 `wecom/bot.ts:onMessage` 入队时写 `userId = external_userid, platform = 'wecom'`，`openId` 留空（企微侧永远不读 `openId`）
- 飞书侧旧调用方（46 个文件）**不立即迁移**：
  - `feishu/bot.ts:handleChat` 读 `msg.openId` 不变 → 行为不变
  - 但在入口处加 1 行：`if (!msg.userId) msg.userId = msg.openId;`（兜底，让老 spool 文件也能被新代码读）
- PR 2 企微侧写入 SpoolMessage 时**只填新字段**，不写 `openId` / `text`（避免误用飞书代码）

**为什么这样安全**：
- 飞书路径：旧 `openId` 字段仍然在，新 `userId` 是冗余副本；零行为变化
- 企微路径：写新字段 `userId / platform`；企微内部所有新代码只用新字段
- 未来要做跨平台 session 关联时，按 `userId + platform` 索引，与 `openId` 字段彻底解耦

### 3.4 多 daemon 并存语义（v1.1 新增，针对 PR 3 StateCoordinator）

**问题**：spec §3.1 描述了 `--platform=all`（单实例多 channel），但**没回答**「飞书 daemon 和企微 daemon 能否在同一台机器上同时跑」。

**决策**（v1.1 明确）：

| 场景 | 行为 | Lock 文件 |
|---|---|---|
| `cc-linker start --platform=feishu`（只跑飞书） | ✅ 允许 | `~/.cc-linker/owner.feishu.lock` |
| `cc-linker start --platform=wecom`（只跑企微） | ✅ 允许 | `~/.cc-linker/owner.wecom.lock` |
| `cc-linker start --platform=all`（单实例双通道） | ✅ 允许 | `~/.cc-linker/owner.lock`（复合 platforms 列表） |
| 同时跑两个 `--platform=feishu` daemon | ❌ 拒绝（assertNotRunning 同 platform） | E013 |
| 同时跑 `--platform=feishu` 和 `--platform=all` | ❌ 拒绝（冲突） | E013 |
| 同时跑 `--platform=feishu` 和 `--platform=wecom` | ✅ 允许（独立 lock） | 各自 lock |

**StateCoordinator 改造（PR 3 Task 3.3）**：
...
- 文件命名：`owner.${platform}.lock` 或 `owner.lock`（--platform=all 时）
- `isLocked()` 改为 `isLocked(platform)` —— 用于 CLI 命令检查「飞书 daemon 是否在跑」

**为什么这样**：
- 单平台锁 + 独立文件 → 多 daemon 并存零冲突
- `--platform=all` 持复合锁 → 阻止任何单平台 daemon 抢同 platform
- 反向也成立：单平台 daemon 持锁 → `--platform=all` 启动会拒绝
...
export type PlatformMessage = {
  ...
  chatType: 'p2p' | 'group';
  chatId: string;              // feishu p2p: open_id; feishu group: chat_id; wecom: chatId (单聊=user, 群聊=群id)
  messageId: string;
  text: string;                // feishu: 原始 content string（JSON parse 在 bot.ts 下游）；wecom: 已解析文本
 ...
 * 平台无关的流式更新接口（最小公共子集）
 *
 * ⚠️ 接口范围：只覆盖"普通聊天流式"场景的 5 个核心方法。
 * 飞书 Agent View / rendezvous 专属方法（setRendezvousShortId / patchWaitingCard /
 * cancelPending / patchAbortedTracking / adoptExistingCard）不在此接口内，
 * 飞书侧这些代码继续直接用 CardUpdater 类型。wecom v1 不涉及 Agent View。
 * 未来 wecom 若要做等价功能，再加 WecomRendezvousUpdater 类。
 */
...
- 把 SDK 抛出的 `WSAuthFailureError`（botId/secret 错）映射为 `emit('fatal', CCLinkerError('E_CONFIG_WECOM_AUTH'))`
- 把 SDK 抛出的 `WSReconnectExhaustedError`（重连耗尽）映射为 `emit('fatal', CCLinkerError('E_CONFIG_WECOM_NETWORK'))`
- **⚠️ 不在 EventEmitter listener 内 throw**：throw 会变成 uncaught exception，绕过 bot 层的 handleError()。fatal 错误统一 emit 结构化 `'fatal'` 事件，由 bot.ts 监听并调 handleError() 走标准退出（含 errors.ts suggestions 展示）

#### `src/wecom/stream-updater.ts` (~150 行)
实现 `StreamUpdater`（接口见 §4.1），用 SDK `replyStream` 流式消息协议
- `startProcessing(userId)` → `wsClient.replyStream(frame, streamId, initialText, false)` → 返回 streamId
- `updateStream(thinking, text, elapsedMs, toolUses)` → `wsClient.replyStream(frame, streamId, merged, false)` → 同 streamId patch
- **节流**：2000ms（30/min 上限保护），可配置（`wecom.stream_throttle_ms`）
- **Content 上限**：20480 bytes（SDK 硬限制）；超长 buffer 截断 + finish 时丢弃
- `complete(response, ...)` → `replyStream(..., true)` 纯文本收尾
- `error(message)` → `replyStream(..., true)` 发 markdown 错误
- `cancel(reason?)` → `replyStream(..., true)` 发 "⏹ 已取消"
- **终态方法前置处理**：`complete` / `error` / `cancel` 先 (1) guard `currentStreamId === null`（幂等）；(2) `clearTimeout(flushTimer)`（避免 timer fire 把终态消息覆盖回上一帧——`CardUpdater.cancelPending` 同坑）
- **限频 buffer**：errcode 45009/45033 → buffer 累积 → finish 时合并到 finalContent；其他错误 rethrow
...
| 文件 | 改动 | 行数 | PR |
|---|---|---|---|
| `src/feishu/stream-updater.ts` | **新增** FeishuStreamUpdater（包装 CardUpdater） | +60 | 1 |
| `src/feishu/bot.ts` | **改造** handleChatStreaming 切到 FeishuStreamUpdater + onMessage 同步填 userId/platform | +30/-30 | 1 |
| `src/queue/spool.ts` | **改造** SpoolMessage 加 `userId + platform` 字段（openId/text 保留 alias） | +5 | 1 |
| `src/feishu/mapping.ts` | **改造** 改为 re-export 兼容层（PlatformUserManager 主体在 platform/） | +10/-260 | 1 |
...
| `src/runtime/state-coordinator.ts` | `tryAcquire(platform)` 单平台锁 + `owner.${platform}.lock`（v1.1 §3.4） | +50 | 3 |
...
WecomStreamUpdater 流式发送（节流 2000ms）：
  t=0s    startProcessing(userId)  → streamId + 首帧 "🤔 思考中..."
  t=2s    updateStream("正在分析...", "", 2000, [])
  t=4s    updateStream("正在分析...", "准备...", 4000, [])
  ...     节流窗口内合并 chunk，最大 ~1KB
  t=N s   complete(finalText, tokensIn, tokensOut, durationMs, numTurns)
...
限频处理（errcode 45009/45033）：
  → 客户端 buffer + skip 下次 updateStream
  → complete 时 buffer 合并到 finalContent
  → warn 日志 + metric
                                  ↓
...
- `WSAuthFailureError`：botId/secret 错导致认证连续失败耗尽 maxAuthFailureAttempts → **不可恢复**
  - 处理：AibotClient.setupListeners 监听到后 **emit `'fatal'` 事件**（不 throw）+ 携带 `CCLinkerError('E_CONFIG_WECOM_AUTH')`。WecomBot 监听 `'fatal'` 事件后调 `handleError(err)` 走标准 fail-fast 流程（errors.ts 的 suggestions 自动展示修复步骤）
  - 日志：error（一次性，附完整 stack）
- `WSReconnectExhaustedError`：WSS 重连次数耗尽 maxReconnectAttempts → 网络持续不可达
  - 处理：同 `WSAuthFailureError`，emit `'fatal'` + `CCLinkerError('E_CONFIG_WECOM_NETWORK')` → handleError 触发 A3 进程自杀
  - 日志：error
- **⚠️ 设计要点**：EventEmitter listener 内 `throw` 是 uncaught exception，会绕过 bot 层的 handleError() 和 errors.ts 的 suggestions 展示。所有 fatal 错误必须走 emit 路径，让 bot 层统一处理。
...
**C1. spawn 失败** — 立即调 `StreamUpdater.error("Claude CLI 启动失败")` + 发"重试 / 帮助" 模板卡片。不重试。

**C2. mid-stream crash** — 参考现有 session._buildStreamingResult 模式
- `StreamUpdater.complete(lastBuffer + "[回复被中断]", ...)` 终态（success=false 通过卡片状态表达）
...
### PR 1：抽象层（platform/）+ 飞书侧切换

**目标**：抽出 PlatformMessage / StreamUpdater 接口 + UserManager 兼容层；飞书普通聊天路径切到 `FeishuStreamUpdater`；**不引入企微代码**

**范围**：
- 新增 `src/platform/types.ts`, `src/platform/stream-updater.ts`
- 新增 `src/platform/user-state.ts`（接口契约 + 跨平台抽象）+ `feishu/mapping.ts` 改为 re-export 兼容层
- 新增 `src/platform/command-handler.ts`（从 feishu 抽公共部分）
- 新增 `src/feishu/stream-updater.ts`（FeishuStreamUpdater 包装 CardUpdater）
- 改造 `src/feishu/bot.ts:handleChatStreaming` 切到 `FeishuStreamUpdater`（**核心**，否则 FeishuStreamUpdater 是死代码）
- 改造 `src/feishu/bot.ts:onMessage` 写 SpoolMessage 时同步填 `userId + platform` 字段
- 改造 `src/queue/spool.ts` SpoolMessage 加 `userId + platform` 字段（openId/text 保留）

**验收**：
- 所有 platform/ 单测通过
- 飞书 `handleChatStreaming` 切到 FeishuStreamUpdater 后行为零变化（5 E2E case 全过）
- `FeishuStreamUpdater` 类在飞书生产代码中被 `handleChatStreaming` 实际消费（不是死代码）
- 飞书所有单测 + 集成测试 100% 通过
- 飞书路径 demo 5 场景手工全过

**风险**：抽取公共代码时漏掉边缘情况 → 飞书 E2E 必跑 + 单测覆盖率 ≥ 90%；CardUpdater 4 个接口外方法（shouldFallbackToText/truncateContent/getCardMessageId/dispose）由 FeishuStreamUpdater 内部保留 CardUpdater 实例访问，不暴露给接口
...
| SDK 抛 `WSAuthFailureError` 误显示 | 配置错时 user 看不懂 | AibotClient 用 `emit('fatal', CCLinkerError)` 替代 listener 内 throw；错误码加 errors.ts suggestion | ✅ 已修复（PR 2 Task 2.2 Step 5） |
