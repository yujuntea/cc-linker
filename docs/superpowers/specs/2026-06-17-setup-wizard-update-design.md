...
2. **飞书命令清单过时**。`printSummary()` 只列出 5 个飞书命令（`/list` `/new` `/switch` `/model` `/status`），但 `bot.ts` 的 `helpText()` 实际支持 13 个。新增的 `listDir` `stop` `agents` 等都没有出现在总结里，初学者根本不知道有这些能力。
...
- 不修改 `bot.ts` 的 `helpText()` 输出（已经是 13 个命令的正确清单）。
...
**Step 编号动态调整：** `totalSteps` 当前在 line 46 算成 `opts.skipFeishu ? 2 : 3`。由于新步骤不依赖 `--skip-feishu` 或 `--skip-hook` 的开关，固定为 `opts.skipFeishu ? 3 : 4`。
...
这样 `config.toml` 输出顺序与 `src/utils/config.ts` DEFAULTS 声明顺序一致，避免落到底部"剩余 sections"块。
...
- 6 个高频命令（按用户决定清单）
- 引导到 `/help` 获取完整列表
- 一句对飞书"自定义菜单"的轻量文案推荐（呼应 Image #15）
- 保持 `chalk.cyan` / `chalk.white` / `chalk.gray` 现有配色

### 4. 不动的部分

- `bot.ts` 的 `handleCommand` switch / `helpText` —— 已经是 13 个命令的正确清单，summary 引导到 `/help` 就够了。
...
| 文件 | 改动 |
|------|------|
| `src/cli/commands/setup.ts` | 新增 Step 2.5 流程；新增 `savePermissionMode()`；修改 `totalSteps` 计算；修改 `printSummary` 命令块 |
...
| `tests/unit/cli/commands/init-feishu.test.ts` | 新增 `saveConfig` 输出顺序测试（确保 `claude`/`sdk` 紧跟 `feishu_bot`） |
...
- `cc-linker start --daemon` 起来后，飞书端发 `/help` 仍返回 13 个命令的完整列表（验证没破坏 bot.ts）
...
**风险 2：[sdk].permission_mode 是 Agent View 的核心字段。**
- 评估：默认值已经是 `acceptEdits`，强写一遍无副作用。但要注意：如果用户有自定义的 `claude.permission_mode` 但 `sdk.permission_mode` 不同，setup 跑完后两边会被强制对齐。这是规约（用户决定"同步"），不算 bug。

**风险 3：summary 文案与 helpText 文案偶有漂移。**
- 评估：summary 是终端一次性输出，helpText 是运行时命令。两者都有 6 个高频命令就够；用户发 `/help` 总能拿到最新最全的。无需做单一来源。
