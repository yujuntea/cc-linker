...
review 报告原始 8 Major + 15 Minor，**主批已修 6 项**（C-1/C-2/C-3/C-4/M-1/M-3/M-7），**M-3.2 补批修了 4 个文件 fsync**，**PR 5.1 修了 M-1 production + C-1+C-2 silent + command path**。**剩余 3 Major + 11 Minor**:
...
**Files:**
- Modify: `src/wecom/bot.ts:430-460` (handleChat 处理 images)
...
`src/wecom/bot.ts:430` handleChat 在 owner 验证后加：
...
**Files:**
- Modify: `src/wecom/bot.ts:200-230` (handleCommand switch case 加 bridge)
...
`src/wecom/bot.ts:200-228` switch case 加：
...
并在 `handleCommandHelp` (bot.ts:300) 帮助文本保留 "/bridge 已废弃" 提示（PR 5 已加）。
...
**关键修正**: PR 5 已实现 `/stop <short>` (wecom/bot.ts:384-398 调 `claude stop <shortId>` via execFile)，**不需要新加 WecomBgSessionStopper 类**。本 Task 仅加边界测试覆盖。
...
- [ ] **Step 2: 实现 retry action** (`src/wecom/bot.ts:662` switch case)
...
- [ ] **Step 2: 实现 stop action** (`src/wecom/bot.ts:662`)
...
**关键修正**: 原 plan 用 bridge.listFeishuSessions, 但 /bridge 已废弃。改用 `src/registry/registry.ts` 的 `RegistryManager.listActive()` 直接读 `~/.cc-linker/registry.json`。
...
Run: `bun test`
Expected: 1172 + 2 (image-handler) + 1 (Task 6.1 bot) + 1 (Task 6.2 bot) + 2 (Task 6.3 bot) + 1 (Task 6.4 bot) + 1 (Task 6.5 bot) + 1 (Task 6.6 bot) + 1 (Task 6.7 bot) = **1180+** 全过
...
- [ ] review 报告原 8 Major 全部修复 ✅ (C-1/C-2/C-3/C-4/M-1/M-3/M-7 + M-3.2 4 files + M-1 production 5.1 fix)
