...
function makeBotWithMocks(opts: {
  ...
  const mockSpoolQueue = {
    ...
    // 其他方法 stub, 按现有 bot.test.ts 风格 (PR 6 用 mock(...))
  };
...
**PR 7.3 review fix #7**: `handleCommandListDir(userId)` 已存在（bot.ts:803-851），返回 markdown string。**不重写** — `renderListDir` 委托给它：
...
**Step 3.1**: **不改** `handleCommand case 'listdir'` (bot.ts:448-449) — 保持原样：

```typescript
        case 'listdir':
          // PR 7.3 fix #7: 保持原样 (responseText 由 handleCommandListDir 返回)
          //   走外层 sendMessage (bot.ts:476-481), 避免双推送
          //   按钮路径 (executeCardAction case 'listdir') 走 renderListDir
          responseText = await this.handleCommandListDir(msg.userId);
          break;
```

> **重要**: renderListDir **只**给按钮路径用 (executeCardAction case 'listdir')，
> 命令路径保持 `responseText = await this.handleCommandListDir(...)` 让外层统一 sendMessage。
> 否则用户会收到**两份重复** listdir 输出（renderListDir 一份 + 外层 responseText 一份）。

- [ ] **Step 4: handleChat 调 updater.complete 时传 completeCtx**

找到 `handleChat` 方法内调 `this.updater.complete(...)` 那行（**实际位置**：bot.ts:1017-1027，签名是 `complete(finalText, tokensIn, tokensOut, durationMs, numTurns, fallback, thinking, toolUses)`）。**加最后参数 + 在 class 加 fallback 字段**：

**Step 4.1**: 在 `WecomBot` class 字段区（已有 `this.updater = ...` line 173 附近）追加：
...
```typescript
  ...
  private setLastSessionMeta(meta: { sessionTitle?: string; sessionUuid?: string; cwd?: string }): void {
    ...
    // PR 7.3 review fix: 完整 ctx 直接传给 updater.complete, 不需要类字段缓存 sessionUuid/cwd
    //   (updater 自己有 lastSessionUuid/lastCwd 字段). 这里只缓存 title (bot 侧需要的元信息)
  }
```

**Step 4.2**: 修改 bot.ts:1017-1027 的 `updater.complete` 调用，加最后参数：
...
```typescript
      await this.updater.complete(
        ...
        // PR 6.21: thinking + toolUses 都传给 complete
        thinking, toolUses,
        // PR 7.3: 完成卡片 ctx
        // 来源: this.lastSessionTitle (setPending 后由 setLastSessionMeta 记录) + result.sessionId + cwd
        {
          sessionTitle: this.lastSessionTitle,
          sessionUuid: result.sessionId,  // ClaudeSessionManager 返回值里有 sessionId 字段 (确认: 是)
          cwd: cwd,
        },
      );
```

> **PR 7.3 review fix #2**: `result.sessionId` 来自 ClaudeSessionManager.sendStreamingMessage 返回值（type Result 含 sessionId），
> 不是凭空假设。实施时若 result.sessionId 为 undefined，传 undefined 即可，complete-card.ts ctx 都 optional。
> `cwd` 是 handleChat 已有的局部变量（session 工作目录）。
...
**PR 7.3 review fix #5**: 这步有配置改动风险，**改前必先备份**：

```bash
# Step 7.1: 备份原始 config
cp ~/.cc-linker/config.toml ~/.cc-linker/config.toml.bak-pr74

# Step 7.2: 改错 bot_id (例: 末尾加 .bak 后缀)
sed -i '' 's/bot_id = ".*"/bot_id = "test-broken-bot-id"/' ~/.cc-linker/config.toml
...
# Step 7.4: 发消息, 验证:
# - 流式 markdown 仍正常输出 (replyStream 走另一通道, 不受影响)
# - 完成卡片 sendMessage 失败仅 log warn, 不抛错 (PR 7.2 测试覆盖过)

# Step 7.5: 验证后立即恢复 (CRITICAL: 不恢复会影响生产 daemon)
mv ~/.cc-linker/config.toml.bak-pr74 ~/.cc-linker/config.toml

# Step 7.6: 重启恢复后的 bot
bun run deploy

# Step 7.7: 验证 bot 正常 (发 "hi" 看是否回复)
```
...
```bash
git commit --allow-empty -m "chore: PR 7.4 E2E verification completed
...
> PR 7.4 无代码改动 — 用 `--allow-empty` 标记 E2E 验证完成。
> PR 7.1-7.3 的代码 commit 已在各自 task 末尾完成, 无需重复。
