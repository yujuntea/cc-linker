...
> **重要心智模型**:`img-proxy` 不让模型直接"看见"图片——它把图片**保存成本地文件**,然后给模型发一段**文本**(图片路径 + 选择工具的提示)。模型按其现有能力挑选合适的工具:已注册的任何图片识别 MCP(如 `mcp__MiniMax__understand_image` 之类),或 `Bash` 调用本地图片识别 CLI(如 `mmx-cli`)。这是绕开纯文本模型 4xx 的核心设计。(`Read` 工具对纯文本模型无效:它读图返回的 image 块模型同样消费不了,且会被本代理再次剥离成提示文本形成空转,故默认模板已明确指引模型不要用 Read。)
...
- [数据存储与路由映射](#数据存储与路由映射)
  - [磁盘布局](#磁盘布局)
  - [`routes.json`](#routesjson-路由表) [路由表](#routesjson-路由表)
  - [路由解析逻辑](#路由解析逻辑)
- [智能安装(smart\_mode)](#智能安装smart_mode)
  - [候选发现 (2 路)](#候选发现-2-路)
  - [模型分类规则](#模型分类规则)
  - [模式行为矩阵](#模式行为矩阵)
...
- [`cc-linker setup`](#cc-linker-setup-一键向导) [一键向导](#cc-linker-setup-一键向导)
- [命令参考](#命令参考)
- [配置文件](#配置文件)
...
3. 把 image block **替换**为一段文本(图片路径 + 提示模型自选识别工具:图片识别 MCP / 本地 CLI)
4. 上游收到纯文本请求,正常处理

**前置依赖**:你的纯文本模型必须具备某种图片识别能力(图片识别 MCP / 本地 CLI 如 `mmx-cli` 之一即可)。注:`Read` 工具不算--它读图返回的 image 块纯文本模型同样无法消费。
...
| `~/.claude/providers/<alias>.json`                    | user-managed 原始 provider 配置 | user      | install 改写 ANTHROPIC\_BASE\_URL |
| `~/.claude/providers/<alias>.json.bak`                | 改写前备份(含原始 BASE\_URL)        | user      | uninstall 后删,uninstall 前保留      |
...
**手动改 token 后再 install**:`.bak` 不动,token 保留;`isProviderInstalled()` 检测到 BASE\_URL 仍是当前 config 的 proxy URL → 真幂等,不重复写。
...
`installProvider` 不是简单的"备份 + 改写 + 加路由", 而是根据当前 BASE\_URL 状态分支:

| 状态                     | 触发                                                                    | 行为                                                                  |
| ---------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------- |
| **真幂等**                | currentUrl 已是当前 config 的 proxy URL (`http://${hostname}:${port}/...`) | 不动文件;只确保路由存在(轮换过的 token / 其它字段保留)                                   |
| **跨 port/hostname 重装** | currentUrl 是任一历史 proxy URL 但不是当前 config 的                             | 更新 BASE\_URL 到当前 config 的 proxy URL;`.bak` 不动(保留原始 upstream);其它字段保留 |
| **首次 install**         | currentUrl 既不是当前也不是历史的 proxy URL                                      | 备份到 `.bak`(不覆盖已有);改写 BASE\_URL;加路由                                  |
...
也就是说: wrapper 只替换 BASE\_URL,你的 hooks / 权限规则 / MCP / 主题等配置不会丢。
...
| 状态                        | 标志                                    | 含义                                                                              |
| ------------------------- | ------------------------------------- | ------------------------------------------------------------------------------- |
| 已装 auto (cc-switch 有最新配置) | `↻ <alias>  已刷新`                      | 整体替换 env(token/model/新增字段), 保持 BASE\_URL 是 proxy URL, 同步更新 routes.json upstream |
| 未装                        | `✅ <alias>  新装`                       | 跟 install 一样: 备份 .bak + 改 BASE\_URL + 加路由                                       |
...
`ANTHROPIC_BASE_URL` 是必需字段,`ANTHROPIC_MODEL` 可选(只是展示用)。AUTH\_TOKEN 看你上游要求。
...
| `cc-linker img-proxy start --daemon`            | 后台启动(parent spawn child, 带 CC\_LINKER\_IMG\_PROXY\_DAEMON=1 env) |
...
如果你想自定义(覆盖默认 2 路径建议),改 template:
...
| **Log**       | 最近 100 条请求表格(默认;带 `?limit=N` 可调), 可按 alias / status / streamStatus / 时间过滤;可手动刷新或选 "Last 1h"。列: Time / Alias / Method / Status / Stream Status / **In / Out / Cache R / Cache C**(token 用量) / Chunks / Bytes / Duration / Stripped |
...
| **Cache**     | 实时显示 **Cache Files** 数 + **Cache Size** (KB), 配 "立即清理所有缓存" 按钮(调用 `cleanupOldCache(0)` 全清 + `resetHealthCache()` 让下次读重算)                            |

**Log 流状态枚举**(streamStatus 过滤器下拉选项):
- `complete` — 请求正常完成
- `upstream_error` — upstream 返错
- `upstream_unreachable` — 连不上 upstream
- `client_aborted` — 客户端断开
- `stalled` — 长无 chunk,触发 `stream_idle_timeout_ms`
- `no_body` — 空响应

### 直接 curl API(debug 用)

Console UI 是 2s polling,但 9 个 endpoint 都能直接 curl(绕过 polling 延迟、便于脚本化):

```bash
PORT=8765
curl -s http://127.0.0.1:$PORT/admin/api/health    # {uptimeMs, pid, routeCount, cacheFiles, cacheBytes}
curl -s http://127.0.0.1:$PORT/admin/api/stats    # Dashboard 全量数据
curl -s 'http://127.0.0.1:$PORT/admin/api/log?limit=20&alias=glm-5.2&sinceMs='"$(($(date +%s)*1000 - 3600000))"
curl -s http://127.0.0.1:$PORT/admin/api/config   # GET 当前 config
curl -s -X POST http://127.0.0.1:$PORT/admin/api/config \
  -H 'content-type: application/json' \
  -d '{"upstream_timeout_ms": 30000}'
curl -s http://127.0.0.1:$PORT/admin/api/routes   # [{alias, upstream, installed_at, disabled}, ...]
curl -s -X POST http://127.0.0.1:$PORT/admin/api/routes/disable \
  -H 'content-type: application/json' \
  -d '{"alias":"glm-5.2"}'
curl -s -X POST http://127.0.0.1:$PORT/admin/api/routes/enable \
  -H 'content-type: application/json' \
  -d '{"alias":"glm-5.2"}'
curl -s -X POST http://127.0.0.1:$PORT/admin/api/cache/clear  # {ok:true, removed:N}
```

所有 endpoint 与 console UI 用同一路径, console 写操作依然会写 audit log(见下)。

### 写操作 audit log

所有 console 的 POST 写操作(`config_update` / `routes_disable` / `routes_enable` / `cache_clear`)都写一行 INFO 到 `~/.cc-linker/img-proxy/img-proxy.log`, payload 字段(`console_action` + 操作的 data + `trigger='console'`)全部平铺在顶层 JSON,例如:

```
[<ISO timestamp>] INFO {"time":"...","console_action":"routes_disable","alias":"glm-5.2","trigger":"console"}
[<ISO timestamp>] INFO {"time":"...","console_action":"config_update","applied":{"upstream_timeout_ms":30000},"trigger":"console"}
[<ISO timestamp>] INFO {"time":"...","console_action":"cache_clear","removed":42,"trigger":"console"}
```

事后查"谁在 console 上点了什么"靠这几行。注意: audit log 走 img-proxy 自己的 writer(ISO timestamp + JSON body), 不走 `src/utils/logger.ts` —— 两者格式不同, 混用会让 Log tab 的 filter 漏判(因为 log-parser 专门 regex img-proxy 的格式)。
...
- 默认 prompt 已指引两种路径(MCP / CLI),看模型 response 选了哪条
...
CC Switch 激活的 provider `X` 在 `~/.cc-linker/auto-providers/X.json` 里**不存在**(还没 sync 过来)或 BASE\_URL 还是上游 URL(没 install):
...
### ⚠️ Gotcha: `content-encoding` 双解压 → 客户端崩 → proxy 误报 upstream\_error
...
**症状**: 把 `port` 从 8765 改成 9000 后, `cc-linker img-proxy status` 显示已装 provider, 但 BASE\_URL 还指 8765。

**实际行为**(`installProvider` 3 态机的"跨 port/hostname 重装"): 重新 `cc-linker img-proxy install --providers <alias>` 即可:

- 检测到 currentUrl 是历史 proxy URL(任意 port/hostname) → 改 BASE\_URL 到当前 config 的 proxy URL
...
**实际行为**: console 的 routes\_disable 是**软禁用**——写 `routes[alias].disabled = true`, proxy 看到就 502(alias gate 是 routes.json 的"uninstall 或 disabled"两种状态, 都不认)。provider 文件 + `.bak` + 路由表 entry 都还在; 想恢复去 console Routes tab 点 Enable, 或 `cc-linker img-proxy install --providers <alias>` 重新走 install 流程(会自动清 disabled 标志)。

**为什么不是 uninstall**: console 是临时调试工具, 软禁用让用户能快速 "关掉某个 alias 看看不剥图的话 upstream 是不是对的", 不希望误删 `.bak` / 改写 BASE\_URL。
...
3. **默认** **`prompt_template`** **已不再硬编码具体 MCP**——它指引模型自选工具(任何图片 MCP / `mmx-cli` 等本地 CLI,不含 `Read`:纯文本模型消费不了 Read 返回的 image 块)。若你模型没回应该提示, 手动改 `prompt_template` 指定你想用的工具名。
...
7. **console Routes tab 显示的** **`disabled`** **字段**——是 console 软禁用; 和"已 install 但 BASE\_URL 没指 proxy URL"(手动改)是两码事, 后者 `isProviderInstalled()` 返 false, 但 routes.json 仍有 entry。
...
| `src/img-proxy/transform.ts`             | 剥 image block(含递归 tool\_result.content)+ 落盘 + `DEFAULT_PROMPT_TEMPLATE`                                                        |
...
| `src/img-proxy/console/html.ts`          | INDEX\_HTML(SPA 5-tab 模板, vanilla JS)                                                                                          |
